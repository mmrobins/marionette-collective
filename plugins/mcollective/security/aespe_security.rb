module MCollective
    module Security
        ...
        class Aespe_security<Base
            def decodemsg(msg, requestid=nil)
                body = deserialize(msg.payload)

                # PE specific behavior to quickly bail out prior to decrypting messages we are not expecting
                if requestid
                    unless requestid == body[:requestid]
                        Log.debug("Quick ignoring message %s that does not match %s" % [body[:requestid], requestid])
                        raise MsgDoesNotMatchRequestID
                    end
                end
